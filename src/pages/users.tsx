...
const visibleColumns = ["identification", "name", "email", "isActive", "actions"];
...
export default function UsersPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        user.identification.toLowerCase().includes(filterValue.toLowerCase()) ||
  ...
  const openEditFormModal = (rowData: UserProps) => {
    setFormData(rowData);
    setIsModalOpen(true);
  };
    ...
    <DefaultLayout>
      <section className="flex flex-col items-center justify-center gap-4 py-8 md:py-10">
        <div className="title-container">
          <h1 className="title-text">USUARIOS</h1>
        </div>
        <Table
          isHeaderSticky
