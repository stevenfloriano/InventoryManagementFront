...
const visibleColumns = ["sku", "name", "price", "stock", "isActive", "actions"];
...
export default function ProductsPage() {
  ...
  const [formData, setFormData] = useState<ProductProps>(initialFormData);
  ...
  const openEditFormModal = (rowData: ProductProps) => {
    setFormData(rowData);
    setIsModalOpen(true);
  };
  ...
  const renderCell = useCallback((row: any, columnKey: any) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User
            avatarProps={{ radius: "lg", src: "https://e7.pngegg.com/pngimages/918/14/png-clipart-brown-box-cardboard-box-corrugated-box-design-carton-closed-cardboard-box-miscellaneous-cardboard-thumbnail.png" }}
    ...
    <DefaultLayout>
      <section className="flex flex-col items-center justify-center gap-4 py-8 md:py-10">
        ...
        <Modal isOpen={isModalOpen} size="5xl" onOpenChange={setIsModalOpen}>
          <ModalContent>
            <ModalHeader>{isEditing ? "Editar Producto" : "Nuevo Producto"}</ModalHeader>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="w-[50%]">
                    <Input
                      ...
                      onValueChange={(val) => setFormData({ ...formData, stock: Number(val) })}
                    />
                  </div>
                </div>
                <Switch
